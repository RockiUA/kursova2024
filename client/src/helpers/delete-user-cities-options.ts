...
import { FETCH_CURRENT_WEATHER_BY_USER_QUERY } from '@graphql/queries/fetch-current-weather-by-user.query';
...
export function deleteUserCitiesOptions(filter: FilterData) {
  const currentData = client.cache.readQuery({
    query: FETCH_CURRENT_WEATHER_BY_USER_QUERY,
  }) as FetchCurrentWeatherByUserQuery;

  const isSameCity = (cacheCity: CityForecast, filterCity: FilterData) => {
    return (
